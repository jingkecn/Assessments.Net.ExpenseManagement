...
public sealed class GetExpensesQueryValidatorTests
{
    ...
    {
        // Arrange
        var query = new GetExpensesQuery(pageNumber, 1);
        ...
        exception.Errors.ShouldContain(e => e.PropertyName == nameof(GetExpensesQuery.PageNumber));
    }
    ...
    {
        // Arrange
        var query = new GetExpensesQuery(1, pageSize);
        var validator = new GetExpensesQueryValidator();
        ...
        // Assert
        var exception = await action.ShouldThrowAsync<ValidationException>();
        exception.Errors.ShouldContain(e => e.PropertyName == nameof(GetExpensesQuery.PageSize));
    }
}
