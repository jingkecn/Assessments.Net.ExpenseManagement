...
public sealed class SubmitEmployeeExpenseCommandHandlerTests : TestBase
{
    [Fact]
    public async Task Handle_Should_ReturnTrue_When_Successful()
    {
        ...
        var command = new SubmitEmployeeExpenseCommand(employee.Id, Guid.NewGuid());
        var handler = new SubmitEmployeeExpenseCommandHandler(ExpenseReadRepository, ExpenseWriteRepository);
    ...
    [Fact]
    public async Task Handle_Should_ThrowDomainException_When_ExpenseDoesNotFound()
    {
        // Arrange
        var employee = EmployeePresets.JaneDoe;
        var command = new SubmitEmployeeExpenseCommand(employee.Id, Guid.NewGuid());
