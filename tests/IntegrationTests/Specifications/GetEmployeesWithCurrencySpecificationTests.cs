...
public sealed class GetEmployeesWithCurrencySpecificationTests(TestContainerFactory factory)
    : TestBaseWithDbMigration(factory)
{
    [Fact]
    public async Task Should_GetEmployeesWithCurrency_When_Applied()
    {
        // Arrange
        var expected = EmployeePresets.PredefinedEmployees;
        var specification = new GetEmployeesWithCurrencySpecification();
